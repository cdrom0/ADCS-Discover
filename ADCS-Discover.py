...
def read_ip_file(file_path: str) -> List[str]:
    """Read IP addresses from a file, ignoring empty lines and invalid entries."""
    try:
...
def check_adcs_server_ldap(ip: str, timeout: int = 5, username: str = None, password: str = None) -> tuple:
    """Check if the given IP hosts an ADCS server by querying LDAP."""
    try:
...
def check_adcs_server_http(ip: str, timeout: int = 5, use_https: bool = False) -> tuple:
    """Check if the given IP hosts an ADCS server by probing HTTP/HTTPS endpoints."""
...
def check_esc_vulnerabilities(ip: str, conn: ldap3.Connection, base_dn: str) -> List[str]:
    """Check for ESC1–ESC8 vulnerabilities using an authenticated LDAP connection."""
    vulnerabilities = []
    try:
        ...
        for entry in conn.entries:
            ...
            # Note: Parsing nTSecurityDescriptor requires complex DACL analysis; simplified check here
...
def main():
    ...
    # Scan for ADCS servers and vulnerabilities
